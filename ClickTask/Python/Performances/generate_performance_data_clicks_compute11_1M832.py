...
if __name__ == "__main__":
    ...
    for assumed_h in list_assumed_h:
        for trial_nb in range(num_trials):
            if trial_nb % 100000 == 0:
                cc = datetime.datetime.now().replace(microsecond=0)
                print('elapsed time hh:mm:ss', cc - aa)
                print('entering trial', trial_nb, 'for rate', assumed_h)
            # initialize seed
            np.random.seed(None)
